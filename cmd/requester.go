...
func requestDefault(options RequestOptions) {
	resp, err := requestWithOptions(options, options.method, options.uri, options.headers)
	if err != nil {
	...
	attachHTTPReplay(&base, options.method, options.uri, options.headers, options.body, options.redirect, options.proxy, options.timeout)
	setGlobalBaseline(resp)
	setTechniqueBaseline("default", resp)
	printResponse(base, "default")
}

func requestWithOptions(options RequestOptions, method, uri string, headers []header) (ResponseInfo, error) {
	body := ""
	if strings.EqualFold(method, options.method) {
		body = options.body
	}
	return requestWithRetryBody(method, uri, headers, body, options.proxy, options.rateLimit, options.timeout, options.redirect)
}
...
func requestMethods(options RequestOptions) {
	...
	for _, line := range lines {
		...
		go func(line string) {
			defer w.Done()
			defer p.done()
			resp, err := requestWithOptions(options, line, options.uri, options.headers)
			if err != nil {
				if errors.Is(err, ErrRateLimited) {
...
func requestMethodsCaseSwitching(options RequestOptions) {
	...
	for _, item := range items {
		...
		go func(item workItem) {
			defer w.Done()
			defer p.done()
			resp, err := requestWithOptions(options, item.method, options.uri, options.headers)
			if err != nil {
				if errors.Is(err, ErrRateLimited) {
					return
				}
...
func requestHeaders(options RequestOptions) {
	...
	for _, item := range combined {
		...
		go func(item struct {
			...
			resp, err := requestWithOptions(options, options.method, options.uri, headers)
			if err != nil {
				if errors.Is(err, ErrRateLimited) {
					return
				}
...
func requestEndPaths(options RequestOptions) {
	...
	for _, line := range lines {
		...
		go func(line string) {
			...
			resp, err := requestWithOptions(options, options.method, joinURL(options.uri, line), options.headers)
			if err != nil {
				if errors.Is(err, ErrRateLimited) {
					return
				}
...
func requestMidPaths(options RequestOptions) {
	...
	for _, line := range lines {
		...
		go func(line string) {
			...
			resp, err := requestWithOptions(options, options.method, fullpath, options.headers)
			if err != nil {
				if errors.Is(err, ErrRateLimited) {
					return
				}
...
func requestDoubleEncoding(options RequestOptions) {
	...
	for _, payload := range payloads {
		...
		go func(payload string) {
			...
			resp, err := requestWithOptions(options, options.method, payload, options.headers)
			if err != nil {
				if errors.Is(err, ErrRateLimited) {
					return
				}
...
func requestPayloadPositions(options RequestOptions) {
	...
	for _, item := range items {
		...
		go func(item workItem) {
			...
			resp, err := requestWithOptions(options, options.method, item.uri, options.headers)
			if err != nil {
				if errors.Is(err, ErrRateLimited) {
					return
				}
...
func setupRequestOptions(uri, proxy, userAgent string, reqHeaders []string, bypassIP, folder, method string, verbose bool, techniques []string, banner, rateLimit bool, timeout int, redirect, randomAgent bool) RequestOptions {
	return setupRequestOptionsWithBody(uri, proxy, userAgent, reqHeaders, bypassIP, folder, method, "", verbose, techniques, banner, rateLimit, timeout, redirect, randomAgent)
}

func setupRequestOptionsWithBody(uri, proxy, userAgent string, reqHeaders []string, bypassIP, folder, method, body string, verbose bool, techniques []string, banner, rateLimit bool, timeout int, redirect, randomAgent bool) RequestOptions {
...
func requester(uri string, proxy string, userAgent string, reqHeaders []string, bypassIP string, folder string, method string, verbose bool, techniques []string, banner bool, rateLimit bool, timeout int, redirect bool, randomAgent bool) {
	requesterWithBody(uri, proxy, userAgent, reqHeaders, bypassIP, folder, method, "", verbose, techniques, banner, rateLimit, timeout, redirect, randomAgent)
}

func requesterWithBody(uri string, proxy string, userAgent string, reqHeaders []string, bypassIP string, folder string, method string, body string, verbose bool, techniques []string, banner bool, rateLimit bool, timeout int, redirect bool, randomAgent bool) {
	setVerbose(verbose)
	setDefaultSc(0)
	setDefaultRespCl(0)
	setFragmentCl(0)
	...
	options := setupRequestOptionsWithBody(uri, proxy, userAgent, reqHeaders, bypassIP, folder, method, body, verbose, techniques, banner, rateLimit, timeout, redirect, randomAgent)
